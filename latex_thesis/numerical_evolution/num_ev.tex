...
However, the angular velocities of BBH-b6, BBH-b7, BBH-b10 manifest the typical oscillations that we have also noticed in the previous discussions.\\
...
We now study the evolution of a binary neutron star made of two equal-mass objects rotating with an quasi-equilibrium initial conditions obtained using the software \textbf{Lorene} [REF]. 
The \\
The hydrodynamic evolution of the neutron stars is accomplished using the ideal fluid approximation, provided with the following equation of state (EOS)
\begin{equation}
\label{eos}
P = K \, \rho ^{\Gamma}
\end{equation}
...
A neutron star is a compact object with a density distribution $\rho$, varying from the barycenter up to the surface. 
We plot in Figure() the trajectory of the neutron star 
