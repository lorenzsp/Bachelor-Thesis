...
The geodesic deviation describes the relative motions of free falling objects and it will show a coordinate-independent measure of the gravitational wave effects. \\
...
Naively, the spacetime is more curved if the ripples of the GW are closer to each others.
Considering the plane wave solution
\[ 
 :R^\mu _{00 \sigma }: = -\dfrac{1}{2} \omega^2 :A^{\T \mu} _{\sigma}: \cos (\omega t)
\]
where $\omega$ is the frequency of the plane wave. 
The curvature is proportional to the square of the frequency, in fact we expect a bigger curvature if the metric oscillates more times per second.
Analogously, the electromagnetic field of an oscillating electric dipole is proportional to the square of the frequency, in fact we expect a bigger  field if the charge oscillates more times per second.
 \\
...
\subsection{The quadrupole nature of gravitational waves}
Since the geodesic deviation cannot distinguish between a zero field and a uniform gravitational field, only a tidal gravitational field does the geodesic deviation accelerate. 

Gravitational waves are variations in the curvature of spacetime
